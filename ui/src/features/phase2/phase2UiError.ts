...
const PHASE2_ERROR_MESSAGES: Record<MvpErrorCode | string, string> = {
  ...
  TEAM_MASTER_INVALID: '主 Agent 无效，请选择一个已上线的 Agent',
  TEAM_MEMBERS_INVALID: '子 Agent 选择无效，请检查成员列表',
...
/** Accepts a thrown error or a bare backend error code. */
export function phase2ErrorMessage(
  err: unknown,
  fallback?: string,
): string {
  ...
  return messageForCode('', fallback);
}
