...
import { CopyOutlined, SearchOutlined, SafetyCertificateOutlined } from '@ant-design/icons';
import { createMarketplaceDraft, listMarketplaceCategories, listMarketplaceResources } from '@/services/marketplace';
import type { MarketplaceDraftResponse, MarketplaceResource, MarketplaceResourceType } from '@/services/marketplace';
      ...
      {loading ? <div className="flex justify-center py-64"><Spin /></div> : resources.length === 0 ? <Empty description="没有匹配的模板" /> : (
        <div className="grid grid-cols-1 gap-16 md:grid-cols-2 xl:grid-cols-3">
          {resources.map((resource) => (
            <Card key={resource.id} hoverable onClick={() => { setSelected(resource); setDraftResult(undefined); }} className="h-full" title={<div className="flex items-center justify-between gap-8"><span>{resource.name}</span><Tag>{resource.type}</Tag></div>}>
              ...
              <div className="flex items-center justify-between"><Typography.Text type="secondary">{resource.category}</Typography.Text><Button type="link" icon={<CopyOutlined />} onClick={(event) => { event.stopPropagation(); void copyDraft(resource); }}>复制草稿</Button></div>
            </Card>
          ))}
        </div>
      )}
      <Modal open={Boolean(selected)} title={selected?.name} footer={null} onCancel={() => { setSelected(undefined); setDraftResult(undefined); }}>
        {selected && <div className="space-y-16">
          ...
          <Button type="primary" loading={draftLoading} icon={<CopyOutlined />} onClick={() => void copyDraft(selected)}>复制为我的草稿</Button>
