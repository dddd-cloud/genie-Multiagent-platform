...
public final class BrowserWorkspacePythonToolFactory {
    ...
    private static final String DESCRIPTION = """
            ...
            代码在当前网页的 Pyodide Web Worker 中执行，不访问后端代码解释器。当前选中的浏览器工作区统一挂载在 /workspace。
            运行已有脚本时使用 command=run_script 和快照中给出的绝对路径（例如 /workspace/test.py）；临时代码使用 command=run_code。
            /workspace 中新增、修改或删除的顶层文件会自动同步回当前工作区。不要把 /workspace 改写成 input/ 或 output/。
            """;
    private static final String INPUT_SCHEMA = """
            {
              ...
              "properties": {
                "command": {
                  ...
                  "description": "list_files 列出文件；read_file 按需读取一个文本文件；run_script 运行已有 Python 文件；run_code 执行临时代码。"
                },
                "file_path": {
                  "type": "string",
                  "description": "read_file 时必填，必须是 /workspace 下的绝对顶层文件路径。"
                },
                "max_chars": {
                  "type": "integer",
                  "minimum": 1,
                ...
                "script_path": {
                  "type": "string",
                  "description": "run_script 时必填，必须是 /workspace 下的绝对 .py 文件路径。"
                },
                "code": {
                  "type": "string",
                  "minLength": 1,
                  "description": "run_code 时必填。代码可直接读写 /workspace；可设置 JSON 可序列化的 result 变量。"
                }
              }
            }
            """;
    static final String SCRIPT = """
            ...
            def _mount_workspace(execution_root):
                ...
                if input_root.exists():
                    for source in input_root.iterdir():
                        if source.is_file():
                            shutil.copyfile(source, WORKSPACE_ROOT / source.name)

            def _sync_workspace_changes(execution_root, initial_files):
                output_root = execution_root / "output"
                output_root.mkdir(parents=True, exist_ok=True)
                current_files = _top_level_files(WORKSPACE_ROOT)
                changed = []
                for name, digest in current_files.items():
                    if initial_files.get(name) != digest:
                        shutil.copyfile(WORKSPACE_ROOT / name, output_root / name)
                        changed.append(name)
            ...
            def _workspace_file(raw_path, suffix=None):
                ...
                if path.parent.resolve() != WORKSPACE_ROOT.resolve() or not path.is_file():
                    raise ValueError("file path must reference an existing top-level workspace file")
            ...
            def main(payload):
                ...
                try:
                    ...
                    initial_files = _top_level_files(WORKSPACE_ROOT)
                        ...
                        if command == "list_files":
                            namespace = {"result": {
                                "files": [
                                    ...
                                    for path in sorted(WORKSPACE_ROOT.iterdir(), key=lambda item: item.name)
                                    if path.is_file()
                                ]
                            }}
