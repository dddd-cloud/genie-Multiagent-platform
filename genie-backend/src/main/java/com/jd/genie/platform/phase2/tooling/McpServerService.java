...
public class McpServerService {
    ...
    private final ObjectMapper objectMapper; private final Clock clock; private final McpClientAdapter client; private final McpUrlPolicy urlPolicy;
    public McpServerService(JdbcTemplate jdbc, CurrentUserProvider currentUser, CredentialEnvelopeService credentials, ObjectMapper objectMapper, Clock clock, McpClientAdapter client, McpUrlPolicy urlPolicy) {
        this.jdbc = jdbc; this.currentUser = currentUser; this.credentials = credentials; this.objectMapper = objectMapper; this.clock = clock; this.client=client; this.urlPolicy=urlPolicy;
    }
    ...
    @Transactional public McpServerResponse update(String id, UpdateMcpServerRequest req) {
        if(req==null || req.version()<0) throw validation(); validate(req.name(),req.serverUrl(),req.authType(),req.authName()); urlPolicy.validate(req.serverUrl()); var u=currentUser.requireCurrentUser(); var existing=raw(u.tenantId(),u.userId(),id); LocalDateTime now=LocalDateTime.now(clock);
    ...
    @Transactional public McpServerResponse test(String id) { var u=currentUser.requireCurrentUser(); Raw s=raw(u.tenantId(),u.userId(),id); String url=jdbc.queryForObject("SELECT server_url FROM mcp_server WHERE id=? AND tenant_id=? AND owner_id=? AND deleted_at IS NULL",String.class,id,u.tenantId(),u.userId()); urlPolicy.validate(url); String status="SUCCESS", code=null; try { client.listTools(url,AuthType.valueOf(s.authType),s.authName,credentials.decrypt(s.credentialEnvelope,u.tenantId(),u.userId(),id,AuthType.valueOf(s.authType))); } catch(Phase2ContractException ex){ status="FAILED"; code=ex.errorCode().name(); } LocalDateTime now=LocalDateTime.now(clock); jdbc.update("UPDATE mcp_server SET last_check_status=?,last_check_code=?,last_checked_at=?,updated_at=? WHERE id=? AND tenant_id=? AND owner_id=? AND deleted_at IS NULL",status,code,now,now,id,u.tenantId(),u.userId()); return one(u.tenantId(),u.userId(),id); }
    @Transactional(noRollbackFor = Phase2ContractException.class) public List<McpToolResponse> refreshTools(String id) {
